...
let fullOpacity = 0.8;
let hasOpacity = [
  1, 2, 3, 4, 5, 9, 10, 11, 12, 13, 14, 15, 21, 22, 23, 24, 25, 26, 27, 28, 29,
...
function setup() {
  ...
  for (let i = 0; i < hasOpacity.length; i++) {
    oimg[hasOpacity[i]] = loadImage(
      "opacity/oimg (" + hasOpacity[i] + ").png",
      loadComplete
    );
  }
...
function draw() {
  if (loadCompleteCount == 104) {
    ...
    if (!horizontalScroll) {
      if (windowWidth / windowHeight > loadingBG.width / loadingBG.height) {
        ...
        image(
          ...
          0,
          windowWidth,
          (loadingBG.height * windowWidth) / loadingBG.width
        );
      } else {
        image(
          loadingBG,
          0,
          0,
          (loadingBG.width * windowHeight) / loadingBG.height,
          windowHeight
        );
      }
    }
    ...
    if (mouseY < height - progressIcon.height * 2) {
      image(
        progressIcon,
        (-x * width * 0.8) / totalw + width * 0.1,
        height - progressIcon.height * 1.05,
        0.05 * width,
        (progressIcon.height * 0.05 * width) / progressIcon.width
      );
    } else {
      image(
        progressIcon,
        (-x * width * 0.8) / totalw + width * 0.1,
        height - progressIcon.height * 1.06,
        0.06 * width,
        (progressIcon.height * 0.06 * width) / progressIcon.width
      );
    }
  } else {
    background("#e6c946");
    ...
    rect(
      width * 0.2,
      height * 0.5 - lbHeight * 0.5,
      lbHeight + (width * 0.6 - lbHeight) * (loadCompleteCount / 102),
      lbHeight,
      lbHeight * 0.5
    );
  }
...
function mouseDragged() {
  if (horizontalScroll && mouseY < height - progressIcon.height * 2) {
    x += -pmouseX + mouseX;
    x += -pmouseY + mouseY;
  } else if (mouseY > height - progressIcon.height * 2) {
    x += -((-pmouseX + mouseX) * totalw * (1 / 0.8)) / width;
  }
}
...
function subf(imgNo, x, y, i, p, ob) {
  ...
  if (mp < windowWidth && mp > -windowWidth) {
    ...
    if (ob) {
      o = map(
        mp,
        windowWidth * (fullOpacity - 0.5),
        windowWidth / 2 + oimg[imgNo].width * ratio * 0.5,
        10,
        0
      );
      for (let g = 0; g < o; g++) {
        image(
          ...
          displayw * i + vel + x + p,
          y,
          oimg[imgNo].width * ratio,
          oimg[imgNo].height * ratio
        );
      }
    } else {
      image(
        ...
        displayw * i + vel + x + p,
        y,
        pimg[imgNo].width * ratio,
        pimg[imgNo].height * ratio
      );
    }
